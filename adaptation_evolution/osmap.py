...
class Map:
    ...
    filename=None
        ...
        # animate the dynamic edge costs
    ...
    def graph_to_arrays(self, G):
        # for use with numba functions, store the entire network in terms of numpy arrays
        self.N = len(G.nodes)
    ...
    def calcModulation(self, t):
        # caculate the modulation using the Fourier series
        for i in range(self.N):
    ...
    def buildTraffic(
        ...
        maxTime=100,                # max time and steps are for normalisation, to ensure no negative costs are generated
        steps=100
    ):
    ...
    def seedShortestPath(self, pheromone=1):
        # deposit pheromones on the best path
    ...
    def resetPheromones(self):
        # reset pheromones for repeat tests with same map and src/dest nodes
