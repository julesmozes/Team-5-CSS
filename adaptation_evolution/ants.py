...
def build_path(map, alpha, beta, max_steps=1000):
    # legacy build path function using networkx
...
def update_pheromones(map, paths, Q, evaporation):
    # legacy update pheromones for networkx
    G = map.G
...
def build_path_numba(
    mapData,            # tuple with numpy arrays for map -> use osmap.getNumbaData()
    alpha,
    beta,
    max_steps=10000
):  
...
def build_path_numba_dynamic(
    mapData,            # tuple with numpy arrays for map -> use osmap.getNumbaDataDynamic()
    alpha,
    beta,
    max_steps=10000
):  
